...
export GIT_REV?=$(shell printf '%s%s' "$$(git rev-parse --verify --short=7 HEAD)" "$$([[ -n "$$(git status --porcelain --untracked-files=no)" ]] && echo '-dirty')")
.PHONY: image
image:
ifeq ($(DEV_IMAGE), true)
	...
	DOCKER_BUILDKIT=1 docker build --build-arg "GIT_REV=$(GIT_REV)" --label "GIT_REV=$(GIT_REV)" --build-arg "GIT_URL=$(GIT_URL)" --label "GIT_URL=$(GIT_URL)" -t $(IMAGE_PREFIX)argo-rollouts:$(IMAGE_TAG) -f Dockerfile.dev ${DIST_DIR}
else
	DOCKER_BUILDKIT=1 docker build --build-arg "GIT_REV=$(GIT_REV)" --label "GIT_REV=$(GIT_REV)" --build-arg "GIT_URL=$(GIT_URL)" --label "GIT_URL=$(GIT_URL)" -t $(IMAGE_PREFIX)argo-rollouts:$(IMAGE_TAG)  .
endif
	@if [ "$(DOCKER_PUSH)" = "true" ] ; then docker push $(IMAGE_PREFIX)argo-rollouts:$(IMAGE_TAG) ; fi
